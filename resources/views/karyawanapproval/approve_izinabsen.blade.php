...
@section('content')
    ...
    <div id="header-section">
        <div class="appHeader bg-primary text-light">
            ...
            <div class="pageTitle">Proses Izin Absen</div>
            <div class="right"></div>
        </div>
    </div>

    <div id="content-section">
        <form action="{{ route('karyawan-approval.izinabsen.storeapprove', Crypt::encrypt($izinabsen->kode_izin)) }}" method="POST" id="frmApprove">
            @csrf
            <div class="info-card">
                <div class="info-header">
                    <div class="avatar">{{ strtoupper(substr($izinabsen->nama_karyawan, 0, 1)) }}</div>
                    <div class="info">
                        <div class="nama">{{ $izinabsen->nama_karyawan }}</div>
                        <div class="meta">{{ $izinabsen->nik }} · {{ $izinabsen->nama_jabatan }}</div>
                    </div>
                </div>
                <div class="detail-row">
                    <span class="lbl">Departemen</span>
                    <span class="val">{{ $izinabsen->nama_dept }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Cabang</span>
                    <span class="val">{{ $izinabsen->nama_cabang }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Periode</span>
                    <span class="val">
                        {{ date('d/m/Y', strtotime($izinabsen->dari)) }} - {{ date('d/m/Y', strtotime($izinabsen->sampai)) }}
                        <br><span class="dur-badge">@php $lama = hitungHari($izinabsen->dari, $izinabsen->sampai); @endphp {{ $lama }} Hari</span>
                    </span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Keterangan</span>
                    <span class="val">{{ $izinabsen->keterangan }}</span>
                </div>
