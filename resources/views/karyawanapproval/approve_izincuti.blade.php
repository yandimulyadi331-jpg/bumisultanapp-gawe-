...
@section('content')
    ...
    <div id="header-section">
        <div class="appHeader bg-primary text-light">
            ...
            <div class="pageTitle">Proses Izin Cuti</div>
            <div class="right"></div>
        </div>
    </div>

    <div id="content-section">
        <form action="{{ route('karyawan-approval.izincuti.storeapprove', Crypt::encrypt($izincuti->kode_izin_cuti)) }}" method="POST" id="frmApprove">
            @csrf
            <div class="info-card">
                <div class="info-header">
                    <div class="avatar">{{ strtoupper(substr($izincuti->nama_karyawan, 0, 1)) }}</div>
                    <div class="info">
                        <div class="nama">{{ $izincuti->nama_karyawan }}</div>
                        <div class="meta">{{ $izincuti->nik }} · {{ $izincuti->nama_jabatan }}</div>
                    </div>
                </div>
                <div class="detail-row">
                    <span class="lbl">Departemen</span>
                    <span class="val">{{ $izincuti->nama_dept }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Cabang</span>
                    <span class="val">{{ $izincuti->nama_cabang }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Periode</span>
                    <span class="val">
                        {{ date('d/m/Y', strtotime($izincuti->dari)) }} - {{ date('d/m/Y', strtotime($izincuti->sampai)) }}
                        <br><span class="dur-badge">@php $lama = hitungHari($izincuti->dari, $izincuti->sampai); @endphp {{ $lama }} Hari</span>
                    </span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Keterangan</span>
                    <span class="val">{{ $izincuti->keterangan }}</span>
                </div>
