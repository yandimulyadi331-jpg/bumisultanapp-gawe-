...
@section('content')
    <style>
        ...
        .item-box .item-top { display: flex; justify-content: space-between; font-weight: 600; font-size: 12.5px; }
        .item-box .item-bot { display: flex; justify-content: space-between; font-size: 11px; color: #888; margin-top: 2px; }
        ...
        .grand-total-box .lbl { font-size: 12px; font-weight: 700; color: #444; }
        .grand-total-box .val { font-size: 16px; font-weight: 800; color: #28c76f; }
        ...
        .catatan-box textarea:focus { outline: none; border-color: #7367f0; }
        ...
        .btn-act.ok { background: #7367f0; color: #fff; }
    ...
    <div id="header-section">
        <div class="appHeader bg-primary text-light">
            ...
            <div class="pageTitle">Proses Reimbursement</div>
            <div class="right"></div>
        </div>
    </div>

    <div id="content-section">
        <form action="{{ route('karyawan-approval.reimbursement.storeapprove', Crypt::encrypt($reimbursement->no_reimbursement)) }}" method="POST" id="frmApprove">
            @csrf
            <div class="info-card">
                <div class="info-header">
                    <div class="avatar">{{ strtoupper(substr($reimbursement->nama_karyawan, 0, 1)) }}</div>
                    <div class="info">
                        <div class="nama">{{ $reimbursement->nama_karyawan }}</div>
                        <div class="meta">{{ $reimbursement->nik_show }} · {{ $reimbursement->nama_jabatan }}</div>
                    </div>
                </div>
                ...
                <div class="detail-row">
                    ...
                    <span class="val">{{ $reimbursement->no_reimbursement }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Tanggal Pengajuan</span>
                    <span class="val">{{ date('d/m/Y', strtotime($reimbursement->tanggal_pengajuan)) }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Departemen</span>
                    <span class="val">{{ $reimbursement->nama_dept }}</span>
                </div>
                <div class="detail-row" style="border-bottom:none">
                    <span class="lbl">Catatan Global</span>
                    <span class="val text-muted fw-normal">{{ $reimbursement->catatan ?: '-' }}</span>
                </div>

                <div class="section-label">Rincian Item Nota</div>
                <div class="item-list">
                    @foreach($details as $d)
                        <div class="item-box">
                            <div class="item-top">
                                <span>{{ $d->nama_jenis }}</span>
                                <span class="text-primary">Rp {{ number_format($d->nominal, 0, ',', '.') }}</span>
                            </div>
                            <div class="item-bot">
                                <span>{{ date('d/m/Y', strtotime($d->tanggal_transaksi)) }}</span>
                                <span class="text-truncate" style="max-width: 150px;">{{ $d->keterangan }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="grand-total-box">
                    ...
                    <span class="val">Rp {{ number_format($reimbursement->total_nominal, 0, ',', '.') }}</span>
                </div>

                <div class="catatan-box">
                    <label>Catatan Approval (Opsional)</label>
                    <textarea name="keterangan" placeholder="Berikan catatan jika diperlukan..."></textarea>
                </div>
            </div>
